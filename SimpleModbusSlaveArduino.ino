...
void setup()
{
  ...
  */
  ...
  modbus_configure(&Serial, 19200, SERIAL_8N1, 2, 2, HOLDING_REGS_SIZE, holdingRegs);    
...
void loop()
{
  ...
  holdingRegs[DATA10] = analogRead(A0); // update data to be read by the master to adjust the PWM
  
  analogWrite(LED, holdingRegs[ADC0_VAL]>>2); // constrain adc value from the arduino master to 255
